...
func expands(s string, f func(string) string) string {
...
func main() {
	...
	if len(os.Args) > 1 {
		for _, arg := range os.Args[1:] {
			...
			if len(pieces) != 2 {
				fmt.Fprintf(os.Stderr, "usage: ex5.9 KEY=VAL ...")
				os.Exit(1)
			}
	...
	if len(unused) > 0 {
		log.Printf("unused binding: %s", strings.Join(unused, " "))
	}
	if len(missing) > 0 {
		log.Printf("missing binding: %s", strings.Join(missing, " "))
	}
}
